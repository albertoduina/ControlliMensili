...
//			MyLog.waitHere();
			...
			// sfruttare il posizionamento gi� effettuato per p2, a patto che in
...
//			lr.writeTable(fileDir + SEQUENZE_FILE, strRiga3);
	...
	private void testSymphony(double medGel_a, double devGel_a, double medGel_b, double devGel_b, double cnrGel_ab) {
		...
		if (medGel_a != rightValue) {
			IJ.log("medGel2 ERRATA " + medGel_a + " anzich� " + rightValue);
			testok = false;
		}
		rightValue = 59.41805613461411;
		if (devGel_a != rightValue) {
			IJ.log("devGel2 ERRATA > " + devGel_a + " anzich� " + rightValue);
			testok = false;

		}
		rightValue = 1603.3101265822784;
		if (medGel_b != rightValue) {
			IJ.log("medGel1 ERRATA  > " + medGel_b + " anzich� " + rightValue);
			testok = false;
		}
		rightValue = 55.54885187603639;
		if (devGel_b != rightValue) {
			IJ.log("devGel1 ERRATA > " + devGel_b + " anzich� " + rightValue);
			testok = false;
		}
