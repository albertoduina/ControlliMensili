...
public class p11rmn_ implements PlugIn, Measurements {
	...
	public static ResultsTable mainUnifor(String path1, String path2, int direzione, double profond, String info10,
		...
		do {
			...
			if (out2 == null) {
				...
				//
				double[] out1 = devStandardNema(imp1, imaDiff, xCenterRoi - enlarge, yCenterRoi - enlarge, sqNEA,
						checkPixels);
				if (step)
	...
	public static double[] referenceSiemens() {
		...
		// SIGNAL ERRATO 131.1526336669922 anzich� 953.6122448979592
		// SNRATIO ERRATO 40.09543329703558 anzich� 291.53433741655306
	...
	public static double[] positionSearch(ImagePlus imp11, boolean autoCalled, int direzioneTabella, double profond,
		...
		if (direzione != 0 && direzione != direzioneTabella) {
			MyLog.waitHere("Rilevata differenza tra directionFinder e direzioneTabella direzione= " + direzione
		...
		if (direzione == 0) {
			// manualRequired = true;
			// who1= 2;
			// se non riesco a decidere, lascio fare alla tabella che � (forse)
			// pi� saggia di me
			direzione = direzioneTabella;
			// manualRequired = true;
		...
		case 4:
			...
			break;
		case 0:
			MyLog.waitHere("Caso impossibile, la direzione 0 � gestita in precedenza");
		}
	 ...
	 * due maggiori. Ci� determina la direzione in cui troviamo il massimo
	...
	public static int directionFinder(ImagePlus imp1, double xMaximum, double yMaximum, boolean silent, int timeout) {
		...
		// imposto ora quattro Roi quadrate, sceglier� le due con la media pi�
		// alta, ed in base a questo decider� la direzione in cui muovere la
		// MROI
		...
		MyLog.logVector(meanx, "meanx");
		MyLog.waitHere("Direzione 0 perch� non entrato nelle altre");
		return 0;
	}
