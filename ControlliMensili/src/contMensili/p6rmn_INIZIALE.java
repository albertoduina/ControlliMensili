...
public class p6rmn_INIZIALE implements PlugIn, Measurements {
...
//		VERSION = className + "_build_" + MyVersion.getVersion() + "_iw2ayv_build_" + MyVersionUtils.getVersion();
	...
	public int manualMenu(int preset, String testDirectory) {
		...
		do {
			...
			case 5:
				...
				new p6rmn_INIZIALE().mainThickness(path, "0", oldPosition, autoCalled, step, verbose, test);
				UtilAyv.afterWork();
				retry = true;
			}
		} while (retry);
	...
	public void selfTestMenu() {
		if (new InputOutput().checkJar(MyConst.TEST_FILE)) {
			...
			case 1: {
				...
				boolean test = true;
				ResultsTable rt = new p6rmn_INIZIALE().mainThickness(path1, autoArgs, vetRefPosition, autoCalled, step,
						verbose, test);
				rt.show("Results");
	...
	public void selfTestSilent() {
		...
		ResultsTable rt = new p6rmn_INIZIALE().mainThickness(path1, autoArgs, vetRefPosition, autoCalled, step, verbose,
				test);
...
//		int d2x = d3x - mra;
		...
		if (slab) {
			...
			do {
				...
				imp1.updateAndDraw();
				if (step) {
					ButtonMessages.ModelessMsg(">> ROI MODIFICABILE <<, quando pronti premere CONTINUA   <51>",
							"CONTINUA");
				}
				...
				if ((ra11 == ra1) && (xRoi1 == c2x) && (yRoi1 == c2y)) {
			...
			for (int i1 = 0; i1 < ra11; i1++) {
				puntiXX1[i1] = (double) xRoi1 + (double) i1 - c3x;
			}
			for (int i1 = 0; i1 < ra12; i1++) {
				puntiXX2[i1] = (double) xRoi2x + i1 - c3x;
			}
...
//		plot10.show();
		...
		// IJ.log(MyLog.qui() + "SINISTRA== " + sotto1);
