...
public class p10rmn_ implements PlugIn, Measurements {
	...
	public int manualMenu(int preset, String testDirectory) {
		...
		boolean step = false;
		boolean fast = false;
		String[] titolo = { "Controllo Uniformit�", "con save UNCOMBINED e ",
				"immagini circolari" };
		do {
	...
	public static ResultsTable mainUnifor(String path1, String path2,
		...
		do {
			...
			// ora per� devo riordinare i valori restituiti da crossing, in modo
			// che il punto di start del profilo sia quello pi� vicino al centro
			// ROI.
	...
	public static double[] positionSearch(ImagePlus imp11, double profond,
			...
			boolean test, boolean fast, int timeout) {
		//
		...
		// il punto che ci interesasa sar� quello con minor distanza dal maxima
		...
		if (!fast && !test || fast && manual) {
			...
			// in
			// precedenza (anche perch� non vengono comunque pi� utilizzate per
			// i
			// calcoli)
			//
	...
	public static double[][] cannyProfileAnalyzer(ImagePlus imp1,
		...
		if (showProfiles) {
			...
			if (peaks1[2].length > 2)
				MyLog.waitHere("Attenzione trovate troppe intersezioni col cerchio, cio� "
						+ peaks1[2].length + "  VERIFICARE");
			if (peaks1[2].length < 2)
				MyLog.waitHere("Attenzione trovata una sola intersezione col cerchio, cio� "
						+ peaks1[2].length + "  VERIFICARE");
