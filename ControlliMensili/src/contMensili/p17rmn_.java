...
//		VERSION = className + "_build_" + MyVersion.getVersion() + "_iw2ayv_build_" + MyVersionUtils.getVersion();
	...
	public static ResultsTable automaticRoiPreparation4(ImagePlus imp1, int diam2, boolean silent, int timeout,
		...
		// altrimenti effettuer� il threshold manuale
		// UTILIZZO 5 diverse strategie (in realt� sono un mix tra i settaggi di
	...
	public static ResultsTable automaticRoiPreparation3(ImagePlus imp1, int diam2, boolean silent, int timeout,
		...
		// strategia migliore, poi interverr� l'operatore a correggere e/o
		// completare le ROI
		...
		if (vetX.length != numRods1) {
			...
			MyLog.waitHere("Sono state trovate " + vetX.length + " RODS anzich� " + numRods1
	...
	public static ImagePlus localizzaCuboCentrale(ImagePlus imp1) {
		...
		// ora analizzo l'immagine cercando il profilo quadro dell'inserto,
		// riempir� l'esterno di nero, in modo da cercare una seconda volta
	...
	public static ImagePlus filtroRisultati(ImagePlus imp1, ResultsTable rt1) {
		...
		ImagePlus imp2 = rat1.execute(imp1, null);
		// ora analizzo l'immagine cercando il profilo quadro dell'inserto,
		// riempir� l'esterno di nero prima di ricercare nuovamente i RATS. I
	...
	public static Roi analizzaRisultati(ImagePlus imp1, PointRoi pr1, int quadrant, ImagePlus imp2, ResultsTable rt1) {
		...
		// calcolo la proiezione sulla retta e andr� a vedere la distanza della
	...
	public static ImagePlus[] strategiaGENERALE(ImagePlus imp1) {
		...
		// viene effettuato il threshold automatico con RATS. Otterr� il cerchio
	...
	void selfTestSilent() {
		...
		if (ok) {
			IJ.log("Il test di p7rmn_ WARP � stato SUPERATO");
		} else {
	...
	private static void msgRedo(int nPunti) {
		IJ.showMessage("--- A T T E N Z I O N E ---",
				"Sono stati selezionati solo " + nPunti + " anzich� 36  punti,\n--- R I F A R E ---");
	}
