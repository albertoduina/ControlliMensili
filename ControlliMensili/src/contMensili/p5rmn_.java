...
//					MyLog.waitHere("errore cancellazione directory " + newdir3);
	...
	private static double[] analyzeProfile(ImagePlus imp1, int ax, int ay, int bx, int by, double dimPixel,
		 ...
		 * Tale cambiamento � dovuto alle modifiche apportate a ij\ImagePlus.java, in
		 * pratica se l'immagine � calibrata la calibrazione viene automaticamente
	...
	private static Line selectProfilePosition(int xPos, int yPos, int len, ImagePlus imp1, boolean profiVert) {
		...
		do {
			...
			int userSelection1 = ButtonMessages.ModelessMsg(
					"Linea su cui verr� calcolata la FWHM, eventualmente" + " riposizionarla e premere CONTINUA   <24>",
					"CONTINUA", "ORIZZ", "VERT");
